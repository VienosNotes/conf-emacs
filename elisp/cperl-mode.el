...
(defun cperl-looks-like-perl6 ()
  ...
  (or
   ...
   (save-excursion
     (beginning-of-buffer)
     (search-forward-regexp "^[\t ]*\\(use v6\\|class[\t ]+.*;\\)" 320 t))))
...
(defvar cperl-imenu--function-name-regexp-perl
  (concat
   "^\\("				; 1 = all
       ...
       "\\|"
          "[ \t]*\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\|token\\|rule\\)?" ; perl6
			 ...
			 (if (and
			      (or
			       ...
			       (and (eq (preceding-char) ?b)
				    (progn
				      (forward-sexp -1)
				      (looking-at "\\(coro\\|sub\\|method\\|submethod\\|\\token\\|rule\\)\\>")))) ; perl6
			      (setq old-indent
				    (nth 1
					 (parse-partial-sexp
		    ...
		    (if (or i2		; Has 2 args
			    (and cperl-fontify-m-as-s
				 (or
				  ...
				  (string-match "^\\(\\(rx\\|[msy]\\|tr\\)\\s *\\(:\\([igcpw]\\|ignorecase\\|global\\|continue\\|pos\\|once\\|words\\|bytes\\|codes\\|graphs\\|langs\\|\\|[0-9]+\\(st\\|nd\\|rd\\|th\\|x\\)\\|ov\\|overlap\\|ex\\|exhaustive\\|rw\\|P5\\|perl5\\|Perl5\\(<[a-zA-Z]+>\\)?\\|nth\\(([0-9]+)\\)?\\|x\\(([0-9]+)\\)?\\)\\s *\\)*\\)$" argument)           ; perl6: rx
...
;;;				       "\\=\\(\\\\.\\|[^][\\\\]\\|\\[:\\^?\sw+:]\\|\\[[^:]\\)*]")
		...
		(goto-char bb))
		   ...
		   ;; ----------------------------------------------------
		   ;; perl6: duplication of quote code match-beginning 10/11
	       ;; rx:modifiers\s*/
	       ((match-beginning 22)
			...
			(setq b1 22
				  ...
				  bb (char-after (1- (match-beginning b1))) ; tmp holder
				  ;; bb == "Not a stringy"
				  bb (if (eq b1 22)		; user variables/whatever
						 (and (memq bb (append "$@%*#_:-&>" nil)) ; $#y)
							  (cond ((eq bb ?-) (eq c ?s)) ; -s file test
									((eq bb ?\:) ; $opt::s
									 (eq (char-after
			...
			(if bb
			  ...
			  ;; e1 means matching-char matcher.
			  ;;
			  ;; perl6: don't misinterpret modifier params as regex delimiters
			  ...
			  (and (if go (looking-at ".\\sw*x")
					 (looking-at "\\sw*x")) ; qr//x
				   (setq is-x-REx t))
			  (if cperl-use-v6 (setq is-x-REx t)) ; perl6: Perl5's /x modifier is default in Perl6
			  (if (null i)
				  ;; Considered as 1arg form
				  (progn
					(cperl-commentify b (point) t)
			  ...
			  ;; and highlight differently
			  (setq is-REx
					(and (string-match "^\\([sm]?\\|qr\\|rx\\)$" argument) ; perl6: rx
						 (or (not (= (length argument) 0))
							 (not (eq c ?\<)))))
			  (if (and is-REx
					   (eq e (+ 2 b))
				...
				(if (or i2				; Has 2 args
						(and cperl-fontify-m-as-s
							 (or
							  ;; (string-match "^\\(m\\|qr\\)$" argument)    ; perl6
							  (string-match "^\\(\\(qr\\|rx\\|[msy]\\|tr\\)\\s *\\(:\\([igcpw]\\|ignorecase\\|global\\|continue\\|pos\\|once\\|words\\|bytes\\|codes\\|graphs\\|langs\\|\\|[0-9]+\\(st\\|nd\\|rd\\|th\\|x\\)\\|ov\\|overlap\\|ex\\|exhaustive\\|rw\\|P5\\|perl5\\|Perl5\\(<[a-zA-Z]+>\\)?\\|nth\\(([0-9]+)\\)?\\|x\\(([0-9]+)\\)?\\)\\s *\\)*\\)$" argument) ; perl6: rx
							  (and (eq 0 (length argument))
								   (not (eq ?\< (char-after b)))))))
					(progn
					  (cperl-postpone-fontification
	  ...
	  (or (eq (point) lim)
		  ...
		  (or
		   (save-excursion
		    ...
		    (or (and (looking-at "\\(else\\|class\\|grammar\\|continue\\|grep\\|map\\|gather\\|async\\|atomically\\|given\\|when\\|default\\|loop\\|for\\|BEGIN\\|END\\|CHECK\\|INIT\\|START\\|FIRST\\|ENTER\\|LEAVE\\|KEEP\\|UNDO\\|NEXT\\|LAST\\|PRE\\|POST\\|CATCH\\|CONTROL\\|\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\token||\rule\\)?\\)\\>")
			     (not (looking-at "\\(\\sw\\|_\\)+::")))
			;; sub f {}
			(progn
			  (cperl-backward-to-noncomment lim)
			  (and (eq (preceding-char) ?b)
			       (progn
				 (forward-sexp -1)
				 (looking-at "\\(coro\\|sub\\|method\\|submethod\\|token\\|rule\\)[ \t\n\f#]")))))) ; perl6
		...
		(beginning-of-line)
		(looking-at "\\s *}?\\s *\\(\\(els\\(e\\s +\\|\\)\\)?if\\|un\\(less\\|til\\)\\|class\\|gather\\|grammar\\|async\\|atomically\\|given\\|wh\\(ile\\|en\\)\\|loop\\|for\\|\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\|token\\|rule\\)?\\)\\>"))))
      (error nil))))
	     ...
	     (concat
	      ...
	      (mapconcat
           ...
           '("if" "until" "while" "elsif" "else" "unless" "for"
             "foreach" "continue" "exit" "die" "fail" "last" "loop" "goto" "next"
             "redo" "return" "local" "exec" "sub" "do" "dump" "use" "our" "state"
             "require" "package" "eval" "my" "where" "BEGIN" "END" "CHECK"
             "INIT" "START" "FIRST" "ENTER" "LEAVE" "KEEP"
             "UNDO" "NEXT" "LAST" "PRE" "POST" "CATCH" "CONTROL"
             "given" "when" "default" "has" "returns" "of" "is" "does" "but"
             "\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\)?"
             "class" "grammar" "rule" "token" "make" "role" "try" "self" "augment" "supersede")
	       "\\|")			; Flow control
	      "\\)\\>") 2)		; was "\\)[ \n\t;():,\|&]"
...
;; vieno: keywords
	    (list
	     (concat
	      ...
	      "new\\|a\\(bs\\|ccept\\|tan2\\|l\\(arm\\|l\\)\\|n\\(d\\|y\\)\\|s\\)\\|"
	      "b\\(in\\(d\\|mode\\)\\|less\\)\\|"
	      "c\\(h\\(r\\(\\|oot\\)\\|dir\\|mod\\|own\\)\\|aller\\|rypt\\|"
	      "lose\\(\\|dir\\)\\|mp\\|o\\(s\\|n\\(tinue\\|nect\\)\\)\\)\\|"
	      "CORE\\|d\\(ie\\|bm\\(close\\|open\\)\\|ump\\)\\|cwd\\|"
	      "e\\(lems\\|rr\\|x\\(p\\|it\\|ec\\)\\|q\\|qv\\|nd\\(p\\(rotoent\\|went\\)\\|"
	    ...
	    (list ;; perl6
	     (concat
	      ...
	      ;; Bool" "Array" "Hash" "IO" "Code" "Routine" "Sub" "Method" "Submethod"
	      ;; Macro" "Rule" "Block" "Bare" "Parametric" "Package" "Module" "Class"
	      ;; Object" "Grammar" "List" "Lazy" "Eager" "Any"
	      ;; Real" "Scalar" "int8" "Socket"
	      "bit\\|int\\|str\\|num\\|ref\\|bool\\|Bit\\|Int\\|Str\\|Num\\|Ref\\|"
	      "Complex\\|Exception\\|Seq\\|Range\\|Set\\|Junction\\|Pair\\|Mapping\\|Signature\\|Capture\\|"
	      "Bool\\|Array\\|Hash\\|IO\\|Code\\|Routine\\|Sub\\|Method\\|Submethod\\|"
	      ...
	      "Buf\\|Callable\\|"
	      "Object\\|Grammar\\|List\\|Lazy\\|Eager\\|Any\\|"
	      "Real\\|Scalar\\|int8\\|Socket\\|True\\|False\\|"
	    ...
	    (list
	     (concat
	      ...
	      ;; "q" "qq" "qw" "qx" "redo" "rx" "reduce" "return" "say" "scalar" "shift"
	      ...
	      "q\\(\\|q\\|w\\|x\\|r\\)\\|rx\\|re\\(turn\\|d\\(o\\|uce\\)\\)\\|s\\(ay\\|pli\\(ce\\|t\\)\\|"
	      "calar\\|t\\(udy\\|ate\\)\\|u\\(b\\|m\\)\\|hift\\|ort\\)\\|t\\(r\\|ie\\|ype\\|aken?\\)\\|"
	    ...
	    (list (concat "\\<\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\|token\\|rule\\)?" ; perl6: multi|proto sub methods
		  ...
		  5 (if cperl-font-lock-multiline
			'(if (eq (char-after (cperl-1- (match-end 0))) ?\{ )
			 ...
			 (if (eq (char-after (cperl-1- (match-end 0))) ?\{ )
			     'font-lock-function-name-face
			   'font-lock-variable-name-face))))
	    '("\\<\\(package\\|class\\|require\\|use\\|import\\|no\\|bootstrap\\|role\\|grammar\\)[ \t]+\\([a-zA-z_][a-zA-z_0-9:]*\\)[ \t;]" ; require A if B;
	    ...
	    (cond ((featurep 'font-lock-extra)
		  ...
		  (font-lock-anchored
		   ...
		   (` ((, (concat "\\<\\(has\\|my\\|local\\|our\\|state\\)"
			...
			(3 font-lock-variable-name-face)))))
;mod		  (t '("^[ \t{}]*\\(has\\|my\\|local\\|our\\|state\\)[ \t]*\\(([ \t]*\\)?\\([$@%*][a-zA-Z0-9_:]+\\)"
...
in subdirectories too."
  ...
  (let ((cmd "etags")
	(args '("-l" "none" "-r"
		...
		"-r"
		"/\\<\\(package\\|class\\)[ \\t]+\\(\\([a-zA-Z0-9:_]*::\\)?\\([a-zA-Z0-9_]+\\)\\)[ \\t]*\\([#;]\\|$\\)/\\2/"; perl6
		"-r"
		"/\\<\\(package\\|class\\)[ \\t]*;/\\1;/"))
	res)
  ...
  (let ((b (get-buffer cperl-tmp-buffer)) ind lst elt pos ret rel
	...
	(while lst
	  ...
	  (if elt
		...
		(insert (elt elt 3)
			127
			(if (string-match "^\\(package\\|class\\) " (car elt)) ; perl6
			    (substring (car elt) 8)
			  (car elt) )
			1
